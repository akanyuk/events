...
echo '<div class="mb-3 badge text-bg-danger">Preview!</div>';
echo display_work_media($Module->record, array('rel' => 'preview'));
?>
<?php ob_start(); ?>
    ...
    <dl>
        <?php if ($isPublished): $permalink = NFW::i()->absolute_path . '/' . $Module->record['event_alias'] . '/' . $Module->record['competition_alias'] . '/' . $Module->record['id']; ?>
            <dt><?php echo $langMain['works permanent link'] ?></dt>
            <dd class="mb-3"><a href="<?php echo $permalink ?>"><?php echo $permalink ?></a></dd>
        <?php endif; ?>
