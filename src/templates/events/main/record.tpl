...
<div class="event-competitions">
    <?php foreach ($competitions as $c) { ?>
        ...
        <h3>
            ...
            <?php else: ?>
                <?php echo $c['position'].'.'.htmlspecialchars($c['title'])?>
            <?php endif; ?>
        </h3>
            ...
            <div class="panel-body">
                <p><?php echo $lang_main['competitions reception']?>:
                    <?php if ($c['reception_from']): ?>
                        <span class="hidden-xs"><strong><?php echo date('d.m.Y H:i', $c['reception_from']).' - '.date('d.m.Y H:i', $c['reception_to'])?></strong></span>
                    <?php endif; ?>
                ...
                <p><?php echo $lang_main['competitions voting']?>:
                    <?php if ($c['voting_from']): ?>
                        <span class="hidden-xs"><strong><?php echo date('d.m.Y H:i', $c['voting_from']).' - '.date('d.m.Y H:i', $c['voting_to'])?></strong></span>
                    <?php endif; ?>
...
<div class="item">
    <div class="header"><a href="#<?php echo $c['alias']?>"><?php echo $c['position'].'. '.htmlspecialchars($c['title'])?></a></div>
