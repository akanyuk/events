...
<script type="text/javascript">
    $(document).ready(function () {
        const submitBtn = $('button[id="add-work"]');
        ...
        // Disable main form submit on open page
        submitBtn.attr('disabled', 'disabled');
    });
</script>
<form id="<?php echo $session_id ?>"
      action="<?php echo NFW::i()->base_path . 'media.php?action=upload&session_id=' . $session_id ?>"
    ...
    <div class="form-group" id="local_file">
        ...
        <div class="col-md-6">
            <div class="alert alert-warning">
                <div class="mb-2"><?php echo $lang_media['MaxFileSize'] ?>:
                    <strong><?php echo number_format($MAX_FILE_SIZE / 1048576, 2, '.', ' ') ?>Mb</strong></div>
                <div class="mb-2"><?php echo $lang_media['MaxSessionSize'] ?>:
                    <strong><?php echo number_format($MAX_SESSION_SIZE / 1048576, 2, '.', ' ') ?>Mb</strong></div>
                <div class="mb-2"><?php echo $lang_media['CurrentSessionSize'] ?>: <strong><span
                            id="session_size">0</span>Mb</strong></div>
            </div>
        </div>
    </div>
